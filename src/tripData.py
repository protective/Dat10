...
if (redo):
	...
	con.query('update ' + TABLE + ' set total_fuel = fuel from (select tid, (max(totalconsumed)-min(totalconsumed))fuel from ' + OLD_TABLE + ' where totalconsumed is not null and totalconsumed < 4000000 group by tid)s where ' + TABLE + '.tid=s.tid;')
...
if (redo):
	...
	con.query('update ' + TABLE + ' set temperature_percentage = p from (select tid, count(case when temperature < 70 and speed > 15 then 1 end)::float/count(*) as p from ' + OLD_TABLE + ' group by tid)f where ' + TABLE +'.tid=f.tid;')
