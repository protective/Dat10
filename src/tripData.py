...
print "Time"
con.query('alter table ' + TABLE + ' drop if exists time')
con.query('alter table ' + TABLE + ' add time int;')
con.query("update "+ TABLE +" set time= t from (select tid, max(extract(epoch from timestamp))- min(extract(epoch from timestamp)) as t from "+ OLD_TABLE +" where dirty is false group by tid)s where "+ TABLE +".tid=s.tid;")
